...
| `OTABulkOperationQueueTests/testCancelStopsProcessing()` | Calling `cancelAll()` from this hosted XCTest case crashes the Xcode 26.3 simulator test host with `malloc: pointer being freed was not allocated`, even with the test isolated from timeout-task cancellation. The other queue tests still run in CI; keep this cancellation lifecycle check local until the hosted runner issue is resolved. |
